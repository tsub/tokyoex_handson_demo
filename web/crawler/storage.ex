defmodule TokyoexHandsonDemo.Crawler.Storage do
  ...
  def has_page?(url) do
    Agent.get(__MODULE__, fn map -> Map.has_key?(map, url) end)
  end

  def put_page(url, pages) do
    Logger.debug "--- Put #{url} ---"
    Agent.update(__MODULE__, fn map -> Map.put(map, url, pages) end)
  end
  ...
  def store_to_db do
    ...
    dump
    |> Enum.each(fn {url, data} -> store_article_to_db(url, data) end)
  end
